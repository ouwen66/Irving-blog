...
## 安装Node.js
Hexo是基于Node.js制作的静态博客，我们要用到Node.js里面的npm(node package manager)包管理器来安装插件，如果你想玩Hexo博客，那么这个Node.js是必装的。
...
## 安装Hexo
安装Hexo，我们需要借助npm(node package manager)这个工具来安装，但是我们国内下载镜像源的速度很慢，所以我们可以先利用npm工具来安装一个cnpm工具(国内的淘宝npm镜像源)，这样一来速度会快很多。
...
## 初始化Hexo
在你的电脑上建一个文件夹，命名没有要求，我这里创建的是blog(为了方便，下文中我都是用blog来代表这个文件夹)，文件夹路径也无所谓，最好装在出了系统盘的固态硬盘中，路径要自己找得到。
...
```
注意：这里的md文件是基于markdown语法进行编辑的，不了解的朋友可以百度学习一下，很简单的，几分钟就能看明白，相信你不是问题。
...
2. 更新完代码可以再localhost:4000预览效果再上传，上传后千万记得更新Gitee Page服务，否则刷新页面会出现没更新的情况
